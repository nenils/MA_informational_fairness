...
def do_it_like_numbers_do(datasetX,target,numerical, categorical):
    ...
    datasetX.rename(columns={'Property_Area_Semiurban': 'Property_Area'}, inplace=True)

    categorical.extend(['Property_Area_Rural', 'Property_Area_Urban'])


    scaler = MinMaxScaler()
    datasetX[numerical] = scaler.fit_transform(datasetX[numerical])
    datasetX[numerical] = datasetX[numerical].astype('float32')
    ...
    datasetX[categorical] = datasetX[categorical].astype('int64')
