...
class Model(namedtuple('Model', 'pred label')):
    ...
    def tpr(self):
        ...
        True positive rate
        """
        return np.mean(np.logical_and(self.pred == 1, self.label == 1))
    ...
    def fpr(self):
        ...
        return np.mean(np.logical_and(self.pred == 1, self.label == 0))

    def tnr(self):
        """
        True negative rate
        """
        return np.mean(np.logical_and(self.pred == 0, self.label == 0))

    def fnr(self):
        """
        False negative rate
        """
        return np.mean(np.logical_and(self.pred == 0, self.label == 1))
